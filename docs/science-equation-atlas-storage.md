...
Each corpus has its own adapter because acquisition semantics differ: Wikidata has entity dumps and P2534 statements, OEIS has a Git export and `%F` lines, PMC uses JATS XML, RFCs use RFCXML/text, and legal/regulatory corpora have their own XML/APIs and versioning rules.

An adapter is responsible only for:

1. discovering and pinning an official snapshot;
...
5. emitting a transient JSONL stream accepted by the common materializer.
...
The canonical v1 Parquet row contract is `data/science-equations/schemas/occurrence-v1.json`.
...
For Wikidata, stream decompression into the adapter and then into the same materializer. For PMC, acquire the licensed JATS subset through the PMC Cloud Service and point the JATS adapter at the materialized XML tree. Avoid giant intermediate JSONL files unless needed for debugging.

The materializer:

- validates the common occurrence fields;
- rejects mixed source/snapshot streams;
...
Mass Parquet shards do not belong in Git history. After external-executor materialization, publish each source snapshot to Internet Archive.
...
The publisher enforces `UPLOAD -> VERIFY -> COMMIT MANIFEST`, never the reverse. It re-hashes local shards before upload, uploads with retries, then checks Internet Archive metadata until every expected file is visible with the expected byte length and MD5 when exposed by the service. SHA-256 remains the Atlas integrity checksum even when the remote service exposes another digest. Existing same-name remote files with conflicting bytes or MD5 abort publication; identical files are reused idempotently.
...
- checksums and verified Internet Archive identifiers/URLs;
