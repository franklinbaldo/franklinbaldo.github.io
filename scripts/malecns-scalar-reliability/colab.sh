...
papers_ref=os.environ["PAPERS_REF"]; release_base=os.environ["RELEASE_BASE"]
root=pathlib.Path("/content/malecns-scalar"); root.mkdir(parents=True, exist_ok=True)
papers=root/"papers"; inputs=root/"inputs"; inputs.mkdir(exist_ok=True); out=root/"out"; out.mkdir(exist_ok=True)
...
graph=inputs/"graph.npz"; urllib.request.urlretrieve(f"{release_base}/graph.npz",graph)
...
summary=json.loads(report.read_text(encoding="utf-8")); summary["papers_ref"]=papers_ref; summary["executor"]="colab"; summary["cache_build_seconds"]=cache_seconds; summary["cached_training_seconds"]=train_seconds; summary["total_seconds"]=cache_seconds+train_seconds
...
PAPERS_REF="$PAPERS_REF" RELEASE_BASE="$RELEASE_BASE" colab "--auth=$AUTH" exec -s "$SESSION" --timeout "${COLAB_EXEC_TIMEOUT:-3600}" -f "$TMP/launcher.py"
