...
OUTPUT_DIR=""
PAPERS_REF="${PAPERS_REF:-bac5eab0aab8b41a348322580d50a5e1a6a7df7d}"
GPU="${COLAB_GPU:-T4}"
...
TRAIN_LIMIT="${FEWNERD_TRAIN_LIMIT:-128}"
DEV_LIMIT="${FEWNERD_DEV_LIMIT:-64}"
EPOCHS="${FEWNERD_EPOCHS:-3}"
...
cache_script=exp/"scripts/fewnerd_semantic_cache_pinned.py"
run(sys.executable,cache_script,"--split","train","--limit",str(train_limit),"--device","cuda","--batch-size","32","--dtype","float16","--output",train_cache,cwd=exp,env=env)
run(sys.executable,cache_script,"--split","validation","--limit",str(dev_limit),"--device","cuda","--batch-size","32","--dtype","float16","--output",dev_cache,cwd=exp,env=env)
report=out/"fewnerd-wholebrain-smoke.json"
cmd=[sys.executable,exp/"scripts/fewnerd_malecns_ner_smoke_pinned.py","--graph",graph,"--train-cache",train_cache,"--dev-cache",dev_cache,"--output",report,"--train-limit",str(train_limit),"--dev-limit",str(dev_limit),"--epochs",str(epochs),"--readout-width","128"]
...
summary["cache_pins"]={"train":json.loads(train_cache.with_suffix('.manifest.json').read_text())["fingerprint"],"dev":json.loads(dev_cache.with_suffix('.manifest.json').read_text())["fingerprint"]}
...
python3 - "$TMP/launcher.py" "$PAPERS_REF" "$RELEASE_BASE" "$TRAIN_LIMIT" "$DEV_LIMIT" "$EPOCHS" <<'PY'
from pathlib import Path
import sys
p=Path(sys.argv[1]); papers_ref=sys.argv[2]; release_base=sys.argv[3]; train=sys.argv[4]; dev=sys.argv[5]; epochs=sys.argv[6]
p.write_text(
    "import os, runpy\n"
