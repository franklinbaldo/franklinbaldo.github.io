...
OUTPUT_DIR=""
PAPERS_REF="${PAPERS_REF:-ac9c3bc7a4dcd36b2615afd1db441e420a6e059a}"
GPU="${COLAB_GPU:-T4}"
...
SESSION="malecns-state-${GITHUB_RUN_ID:-$$}-${GITHUB_RUN_ATTEMPT:-1}"; SESSION="${SESSION,,}"
TMP="$(mktemp -d)"
...
root=pathlib.Path("/content/malecns-state")
...
report=out/"state-conditioned-coupling.json"
t1=time.perf_counter()
run(sys.executable,exp/"scripts/smoke_state_conditioned_coupling_batch_gpu.py","--graph",graph,"--embedding-cache",cache,"--output",report,"--epochs","3","--lr","0.0005","--anchor-lambda","0.05","--readout-width","128",cwd=exp,env=env)
...
shutil.make_archive("/content/malecns-state-result","zip",out)
print(json.dumps({"event":"state_conditioned_colab_complete","cache_build_seconds":cache_seconds,"cached_training_seconds":train_seconds,"promotion_checks":summary.get("promotion_checks")}),flush=True)
PY
...
colab "--auth=$AUTH" download -s "$SESSION" /content/malecns-state-result.zip "$TMP/result.zip"
unzip -q "$TMP/result.zip" -d "$OUTPUT_DIR"
[[ -f "$OUTPUT_DIR/github-summary.json" ]] || { echo "missing Colab summary" >&2; exit 1; }
echo "Colab state-conditioned result: $OUTPUT_DIR"
