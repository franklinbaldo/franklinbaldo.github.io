...
OUTPUT_DIR=""
PAPERS_REF="${PAPERS_REF:-ceb353732b23dc948415b7f0ea5b948448c9e606}"
GPU="${COLAB_GPU:-T4}"
...
SESSION="malecns-dose-${GITHUB_RUN_ID:-$$}-${GITHUB_RUN_ATTEMPT:-1}"; SESSION="${SESSION,,}"
TMP="$(mktemp -d)"
...
root=pathlib.Path("/content/malecns-dose")
...
report=out/"scalar-reliability-dose-response.json"
t1=time.perf_counter()
run(sys.executable,exp/"scripts/smoke_scalar_reliability_dose_response_cached_batch_gpu.py","--graph",graph,"--embedding-cache",cache,"--output",report,"--epochs","3","--lr","0.0005","--anchor-lambda","0.05","--readout-width","128",cwd=exp,env=env)
...
shutil.make_archive("/content/malecns-dose-result","zip",out)
print(json.dumps({"event":"dose_response_colab_complete","cache_build_seconds":cache_seconds,"cached_training_seconds":train_seconds,"selected_followup_arm":summary.get("selected_followup_arm")}),flush=True)
PY
...
colab "--auth=$AUTH" exec -s "$SESSION" --timeout "${COLAB_EXEC_TIMEOUT:-5400}" -f "$TMP/launcher.py"
colab "--auth=$AUTH" download -s "$SESSION" /content/malecns-dose-result.zip "$TMP/result.zip"
unzip -q "$TMP/result.zip" -d "$OUTPUT_DIR"
[[ -f "$OUTPUT_DIR/github-summary.json" ]] || { echo "missing Colab summary" >&2; exit 1; }
echo "Colab dose-response result: $OUTPUT_DIR"
