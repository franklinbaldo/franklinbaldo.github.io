...
OUTPUT_DIR=""
PAPERS_REF="${PAPERS_REF:-7ec9895d1d7c33e22757a1959888dc2ef5dea117}"
GPU="${COLAB_GPU:-T4}"
...
SESSION="malecns-state-directed-${GITHUB_RUN_ID:-$$}-${GITHUB_RUN_ATTEMPT:-1}"
SESSION="${SESSION,,}"
TMP="$(mktemp -d)"
...
root=pathlib.Path("/content/malecns-state-directed")
...
run(
    sys.executable,
    exp/"scripts/smoke_fully_state_conditioned_directed_plasticity_batch_gpu.py",
    "--graph",graph,
...
(out/"github-summary.json").write_text(
    json.dumps(summary,indent=2,ensure_ascii=False,sort_keys=True)+"\n",
    encoding="utf-8",
)
shutil.make_archive("/content/malecns-state-directed-result","zip",out)
print(
    json.dumps(
        {
            "event":"fully_state_directed_colab_complete",
            "cache_build_seconds":cache_seconds,
            "cached_training_seconds":train_seconds,
...
echo "Colab fully state-conditioned directed result: $OUTPUT_DIR"
