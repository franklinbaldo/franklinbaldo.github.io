...
KERNEL_ID="${KAGGLE_MALECNS_STATIONARY_SCREEN_KERNEL_ID:-}"
...
scratch = pathlib.Path("/kaggle/temp/malecns-stationary-screen")
...
calibration_out = work / "stationary-screen"
public_cache = work / "public-cache"
runtime_out.mkdir(exist_ok=True)
calibration_out.mkdir(exist_ok=True)
...
run(
    ...
    "--visual-scale", "0.5",
    "--reward-scale", "0.08",
    "--reward-feedback-gain", "1000",
    "--mutation-sigma", "0.18",
    "--telemetry-every", "100",
    cwd=visual_exp,
)

summary_path = calibration_out / "screen-reward-summary.json"
summary = json.loads(summary_path.read_text(encoding="utf-8"))
summary["visual_ref"] = visual_ref
summary["runtime_ref"] = runtime_ref
summary["kernel"] = kernel_id
summary["kernel_url"] = f"https://www.kaggle.com/code/{kernel_id}"
summary["kernel_visibility"] = "public"
...
provenance = {
    ...
    "interface_manifest": summary["interface_manifest"],
    "screen_geometry_manifest": summary["screen_geometry_manifest"],
}
...
for src, name in (
    (work / "github-summary.json", "github-summary.json"),
    (calibration_out / "progress.jsonl", "progress.jsonl"),
    (calibration_out / "winner-retina.txt", "winner-retina.txt"),
    (calibration_out / "winner-top-receptors.json", "winner-top-receptors.json"),
    (interface_manifest, "visual-interface.json"),
    (screen_geometry_manifest, "screen-geometry.json"),
    (work / "provenance.json", "provenance.json"),
):
...
kaggle kernels push -p "$STAGE" --accelerator "$ACCELERATOR" -t "${KAGGLE_MALECNS_SCREEN_TIMEOUT:-21600}"
echo "Public Kaggle kernel: https://www.kaggle.com/code/$KERNEL_ID"

# Stream the retina grids and reward telemetry into GitHub Actions while the
# public kernel runs. A transport/log-stream failure does not rerun compute.
kaggle kernels logs "$KERNEL_ID" --follow --interval "${KAGGLE_SCREEN_LOG_INTERVAL:-10}" || true

deadline=$(( $(date +%s) + ${KAGGLE_MALECNS_SCREEN_WAIT_SECONDS:-21600} ))
while :; do
...
for attempt in $(seq 1 10); do
  rm -rf "${DOWNLOAD:?}"/*
  if kaggle kernels output "$KERNEL_ID" -p "$DOWNLOAD" -o --file-pattern '.*(github-summary[.]json|progress[.]jsonl|winner-retina[.]txt|winner-top-receptors[.]json|visual-interface[.]json|screen-geometry[.]json|provenance[.]json)$'; then
    downloaded=1
    break
  fi
  sleep "$delay"
...
for name in github-summary.json progress.jsonl winner-retina.txt winner-top-receptors.json visual-interface.json screen-geometry.json provenance.json; do
