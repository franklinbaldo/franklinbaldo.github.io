...
- `audiobook-editorial-segment/SKILL.md` — advance exactly one canonical source -> translation -> narration editorial unit, preserving identity, lineage, project prerequisites and readiness boundaries.
- `audiobook-tts-payload/SKILL.md` — create/review narration shards under the executable `tts-body-v1` contract: control information in frontmatter, exact synthesizer payload in the body.

Always read `audiobook-factory/SKILL.md` first. The narrower skills refine specific stages.

These skills explain how to perform the work. `scripts/audiobook/validate-okf.py` remains the mechanical enforcement boundary. Specs, architecture notes and benchmark reports under `docs/okf/audiobook/` remain the normative/evidentiary project documentation. When new benchmark evidence changes model selection or segmentation policy, update the evidence document and this entry-point skill so future agents do not depend on chat memory.
