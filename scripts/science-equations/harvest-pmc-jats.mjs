...
function attrValue(attrs, name) {
  ...
  const match = attrs.match(new RegExp(`(?:^|\\s)${escaped}\\s*=\\s*(["'])([\\s\\S]*?)\\1`, "i"));
...
function firstTag(xml, tagName, attributeName = null, attributeValue = null) {
  ...
  if (attributeName && attrValue(match[1], attributeName) !== attributeValue) return null;
...
function findArticleId(xml, type) {
  ...
  for (const match of xml.matchAll(regex)) {
    if ((attrValue(match[1], "pub-id-type") ?? "").toLowerCase() === type.toLowerCase()) {
...
export function classifyPmcLicense(xml) {
  ...
  if (!licenseMatch) {
    return { license_url: null, license_text: null, license_id: null, redistribution: "manual-review" };
  }

  const attrs = licenseMatch[1];
  const body = licenseMatch[2];
  const href = attrValue(attrs, "xlink:href") ?? attrValue(attrs, "href") ??
    attrValue((body.match(/<ext-link\b([^>]*)>/i) ?? [])[1] ?? "", "xlink:href") ?? null;
  const licenseText = textOnly(body);
  const haystack = `${href ?? ""} ${licenseText}`.toLowerCase();

  const patterns = [
    ["CC0", /creativecommons\.org\/(?:publicdomain\/zero|licenses\/zero)\/|\bcc0\b/],
    ["CC-BY-NC-SA", /creativecommons\.org\/licenses\/by-nc-sa\/|\bcc\s*by-nc-sa\b/],
    ["CC-BY-NC-ND", /creativecommons\.org\/licenses\/by-nc-nd\/|\bcc\s*by-nc-nd\b/],
...
function extractExpression(inner) {
  ...
  if (tex) {
    return { expression_original: decodeXmlText(tex[1]).trim(), expression_encoding: "tex-math" };
  }

  const mathml = inner.match(/<(?:mml:)?math\b[^>]*>[\s\S]*?<\/(?:mml:)?math>/i);
  if (mathml) {
    return { expression_original: mathml[0], expression_encoding: "mathml" };
  }

  return { expression_original: textOnly(inner), expression_encoding: "jats-text" };
}
...
export function parseJatsArticle(xml, { snapshot, objectPath, redistributableOnly = true } = {}) {
  ...
  const pmcid = pmcidRaw ? (pmcidRaw.startsWith("PMC") ? pmcidRaw : `PMC${pmcidRaw}`) : null;
  const doi = findArticleId(xml, "doi");
  const title = articleTitle(xml);
  const license = classifyPmcLicense(xml);
  const rejectedByLicense = redistributableOnly && license.redistribution !== "redistributable-derived";

  const records = [];
  if (rejectedByLicense) {
    return { records, metrics: { formulas_seen: 0, records_written: 0, rejected_license: 1, rejected_empty: 0 }, article: { pmcid, doi, title, ...license } };
  }
  ...
  for (const match of xml.matchAll(regex)) {
    ...
    const { expression_original, expression_encoding } = extractExpression(inner);
    if (!expression_original) {
      rejectedEmpty += 1;
      continue;
    }

    const sourceLocator = attrValue(attrs, "id") ?? `${tagName}:${formulasSeen}`;
    ...
    records.push({
      ...
      source_document_url: pmcid ? `https://pmc.ncbi.nlm.nih.gov/articles/${pmcid}/` : null,
...
function parseArgs(argv) {
  const args = { root: null, pathsFrom: null, snapshot: null, redistributableOnly: true, limit: Infinity };
  for (let i = 0; i < argv.length; i += 1) {
    const token = argv[i];
    const next = argv[i + 1];
    if (token === "--root" && next) { args.root = next; i += 1; }
    else if (token === "--paths-from" && next) { args.pathsFrom = next; i += 1; }
    else if (token === "--snapshot" && next) { args.snapshot = next; i += 1; }
    else if (token === "--include-manual-review") args.redistributableOnly = false;
    else if (token === "--limit" && next) { args.limit = Number.parseInt(next, 10); i += 1; }
    else if (token === "--help" || token === "-h") args.help = true;
    else throw new Error(`Unknown or incomplete argument: ${token}`);
  }
  if (args.root && args.pathsFrom) throw new Error("choose only one of --root or --paths-from");
  return args;
}
...
export async function harvestPmcFiles({ paths, snapshot, output, redistributableOnly = true, limit = Infinity }) {
  const metrics = { articles_seen: 0, formulas_seen: 0, records_written: 0, rejected_license: 0, rejected_empty: 0 };
  for await (const file of paths) {
    if (metrics.records_written >= limit) break;
    const xml = await readFile(file, "utf8");
    metrics.articles_seen += 1;
    const parsed = parseJatsArticle(xml, { snapshot, objectPath: file, redistributableOnly });
...
async function main() {
  ...
  if (!args.root && !args.pathsFrom) throw new Error("--root or --paths-from is required");
  ...
  const metrics = await harvestPmcFiles({ paths, snapshot: args.snapshot, output: process.stdout, redistributableOnly: args.redistributableOnly, limit: args.limit });
  process.stderr.write(`${JSON.stringify({ event: "harvest-complete", source_id: SOURCE_ID, source_snapshot: args.snapshot, ...metrics })}\n`);
}
