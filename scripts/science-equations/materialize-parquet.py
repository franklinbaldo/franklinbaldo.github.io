...
# ///
"""Materialize a source adapter JSONL stream as canonical Equation Atlas Parquet.

Source-specific adapters are responsible for acquisition and faithful extraction.
This stage is source-agnostic: it validates the common occurrence contract,
preserves the full adapter row in ``source_payload_json``, writes deterministic
Parquet shard names, and emits a manifest with row counts and checksums.

JSONL is deliberately a transient interchange stream. Parquet is the canonical
mass-storage representation of the Atlas equation lake.
"""
...
def normalize_occurrence(row: dict[str, Any], *, expected_source: str | None, expected_snapshot: str | None) -> dict[str, str | None]:
    ...
    source_locator = first_nonempty(row, "source_record_url", "source_statement_id")
    ...
    occurrence_seed = "\n".join(
        [source_id, source_snapshot, source_record_sha256, source_locator or "", source_document_id or ""]
    )
...
def materialize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "shards": shards,
        "internet_archive": {
            "identifier": args.archive_identifier
            or f"scientific-equation-atlas-{source_slug}-{snapshot_slug}",
            "published": False,
        },
    }
