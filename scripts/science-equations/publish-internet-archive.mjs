...
function remotePlan(metadata, expected) {
  ...
  for (const local of expected) {
    ...
    if (Number(found.size) !== local.bytes) {
      throw new Error(`immutable snapshot conflict for ${local.name}: remote bytes ${found.size} != local ${local.bytes}`);
    }
    if (found.md5 && found.md5 !== local.md5) {
      throw new Error(`immutable snapshot conflict for ${local.name}: remote MD5 ${found.md5} != local ${local.md5}`);
    }
  }
  return missing;
}
...
async function main() {
  ...
  const publicationOutput = arg("--publication-output");
  ...
  if (!["extracted", "normalized", "deduplicated"].includes(manifest.stage)) {
  ...
  if (publicationOutput) {
    fs.mkdirSync(path.dirname(path.resolve(publicationOutput)), { recursive: true });
    fs.writeFileSync(path.resolve(publicationOutput), serialized);
  }
