...
KERNEL_ID="${KAGGLE_MALECNS_WHOLEBRAIN_LEARNING_KERNEL_ID:-}"
PAPERS_REF="${PAPERS_REF:-dca01cf11cc0e294d62777486af261168b60dd1b}"
...
EPOCHS="${MALECNS_WHOLEBRAIN_EPOCHS:-20}"
...
if [[ -z "$KERNEL_ID" ]]; then
  KERNEL_ID="${KAGGLE_USERNAME}/malecns-whole-brain-flavour-learning"
fi
...
scratch = pathlib.Path("/kaggle/temp/malecns-wholebrain-learning")
scratch.mkdir(parents=True, exist_ok=True)
repo = scratch / "papers"
inputs = scratch / "inputs"
inputs.mkdir(exist_ok=True)
out = work / "wholebrain-learning"
...
report = out / "learning-curve.json"
progress = out / "progress.jsonl"
run(
    sys.executable,
    exp / "scripts/explore_wholebrain_flavour_curve_gpu.py",
    "--features", features,
    "--graph", graph,
    "--output", report,
    "--progress", progress,
...
for src_path, name in (
    ...
    shutil.copy2(src_path, public_cache / name)
print(json.dumps({"event": "public_cache_ready", "files": sorted(p.name for p in public_cache.iterdir())}), flush=True)
PY

python3 - "$STAGE/job.py" "$PAPERS_REF" "$KERNEL_ID" "$RELEASE_BASE" "$EXPECTED_FEATURES_HASH" "$EXPECTED_GRAPH_HASH" "$EPOCHS" <<'PY'
...
prefix = (
    ...
    f"os.environ['EPOCHS'] = {sys.argv[7]!r}\n"
)
...
{
  "id": "$KERNEL_ID",
  "title": "MaleCNS Whole Brain Flavour Learning",
  "code_file": "job.py",
  "language": "python",
  "kernel_type": "script",
  "is_private": false,
...
kaggle kernels push -p "$STAGE" --accelerator "$ACCELERATOR" -t "${KAGGLE_MALECNS_WHOLEBRAIN_LEARNING_TIMEOUT:-21600}"
echo "Public Kaggle kernel: https://www.kaggle.com/code/$KERNEL_ID"

kaggle kernels logs "$KERNEL_ID" --follow --interval "${KAGGLE_WHOLEBRAIN_LEARNING_LOG_INTERVAL:-10}" || true

deadline=$(( $(date +%s) + ${KAGGLE_MALECNS_WHOLEBRAIN_LEARNING_WAIT_SECONDS:-21600} ))
while :; do
...
for attempt in $(seq 1 10); do
  ...
  if kaggle kernels output "$KERNEL_ID" -p "$DOWNLOAD" -o \
      --file-pattern '.*(github-summary[.]json|learning-curve[.]json|progress[.]jsonl|provenance[.]json)$'; then
    downloaded=1
    break
  fi
...
for name in github-summary.json learning-curve.json progress.jsonl provenance.json; do
