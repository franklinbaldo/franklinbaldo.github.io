...
export type PlayerAction = TradeAction | { type: "investigate"; kind: InvestigationKind };
...
const DEV_LINES: Record<string, string[]> = {
  ...
  silence: ["...", "brb, handling something off-chat", "Need an hour. Do not speculate."],
  celebrate: [
...
function addFeed(state: GameState, kind: FeedKind, label: string, text: string): void {
  state.eventSeq += 1;
...
function applyTrade(state: GameState, action: TradeAction): void {
  ...
  if (action.type === "buy") {
    ...
    state.player.costBasis += spend;
    state.market.price *= 1 + Math.min(0.025, spend / Math.max(state.market.liquidity, 1));
...
function investigate(state: GameState, kind: InvestigationKind): boolean {
  if (state.player.attention <= 0) {
    addFeed(state, "system", "ATTENTION", "No investigation charge available yet.");
    return false;
  }
  ...
  switch (kind) {
    case "wallet": {
      ...
      const message = moved > 0
        ? `Creator-linked wallets moved about $${Math.round(moved).toLocaleString("en-US")} this run. ${
            moved > 18_000 ? "Several transfers lead toward liquid venues." : "Most destinations are still ambiguous."
          }`
    ...
    case "liquidity": {
      const lockQuality = state.creator.archetype === "sociopath" ? "short and partial" : "mixed";
      addFeed(
        state,
        "research",
        "LP CHECK",
    ...
    case "creator": {
      const clue = state.creator.archetype === "sociopath"
...
function updateCreator(state: GameState): void {
  ...
  creator.suspicionEstimate = clamp(creator.suspicionEstimate * 0.72 + market.suspicion * 0.28);

  if (creator.archetype === "desperate_dev") {
    creator.cashPressure = clamp(creator.cashPressure + 0.018 + (market.confidence < 0.35 ? 0.02 : 0));
    if (creator.cashPressure > 0.9 && creator.intent === "build") {
      creator.intent = "delay_then_extract";
      note(state, "Cash pressure crossed the point where taking treasury money started to feel justified.");
    }
  }

  if (creator.archetype === "accidental_success" && !creator.rugAbandoned) {
    if (market.hype > 0.8 && market.liquidity > 275_000 && creator.attachment > 0.45) {
      creator.rugAbandoned = true;
      creator.intent = "build";
      note(state, "The project became worth more as an identity than as a one-time exit. Rug plan abandoned.");
  ...
  if (creator.archetype === "true_believer" && market.price < 0.42) {
    ...
    if (creator.fear > 0.82) {
      ...
      note(state, "Belief remains sincere, but panic is now driving decisions.");
    }
  }
}
...
function chooseCreatorAction(state: GameState): CreatorAction {
  ...
  if (canExtract && thresholdReached && creator.suspicionEstimate < 0.76) {
    ...
    if (roll < extractionChance) {
      return creator.archetype === "sociopath" && market.liquidity > creator.targetLiquidity * 1.12
        ? "remove_liquidity"
        : "staged_dump";
    }
  }
  ...
  if (creator.archetype === "true_believer" && roll < 0.38) return "ship_update";
  if (creator.archetype === "accidental_success" && market.hype > 0.72 && roll < 0.5) return "celebrate";
...
function applyCreatorAction(state: GameState, action: CreatorAction): void {
  ...
  switch (action) {
    case "tease":
      ...
      if (!creator.hasRealCexContact) note(state, "Used an ambiguous future-announcement tease without a concrete exchange deal.");
      break;
    case "reassure":
      addFeed(state, "dev", "DEV", pick(state, DEV_LINES.reassure));
      market.confidence = clamp(market.confidence + 0.04 + creator.deception * 0.025);
      market.suspicion = clamp(market.suspicion - 0.025);
      note(state, `Reassurance chosen because estimated holder suspicion was ${creator.suspicionEstimate.toFixed(2)}.`);
      break;
    case "argue_fud":
    ...
    case "ship_update":
      ...
      note(state, "Shipped a real project artifact; this message is materially grounded.");
      break;
    case "silence":
    ...
    case "wallet_transfer": {
      ...
      note(state, `Moved about $${Math.round(amount).toLocaleString("en-US")} out of the creator wallet.`);
      break;
    }
    case "staged_dump": {
      const fraction = creator.intent === "panic" ? 0.18 : 0.12 + creator.greed * 0.12;
      ...
      addFeed(state, "dev", "DEV", pick(state, DEV_LINES.dump_cover));
      note(state, `Executed a staged sell worth about $${Math.round(amount).toLocaleString("en-US")}.`);
      if (creator.archetype === "desperate_dev" && creator.intent !== "panic") {
        creator.intent = market.suspicion > 0.75 ? "panic" : "delay_then_extract";
      }
      break;
    }
    case "remove_liquidity": {
      ...
      note(state, `Removed ${Math.round(removed).toLocaleString("en-US")} of liquidity and ended the game.`);
      state.status = "rugged";
      break;
    }
    case "celebrate":
...
function moveMarket(state: GameState): void {
  ...
  market.volume = Math.max(4_000, market.volume * (0.82 + random(state) * 0.44) + market.hype * 8_000);
  market.holders = Math.max(140, Math.round(market.holders + (market.hype - market.suspicion) * 115 + (random(state) - 0.5) * 90));
  market.liquidity = Math.max(
    500,
    market.liquidity + (market.confidence - 0.42) * 6_500 + (random(state) - 0.45) * 4_400
  );
  market.hype = clamp(market.hype * 0.93 + random(state) * 0.055);
  market.confidence = clamp(market.confidence * 0.965 + (random(state) - 0.48) * 0.035);
  market.suspicion = clamp(market.suspicion * 0.95 + (random(state) - 0.52) * 0.025);

  if (creator.archetype === "true_believer") market.confidence = clamp(market.confidence + 0.008);
  if (creator.archetype === "sociopath" && market.liquidity > creator.targetLiquidity * 0.8) {
    market.hype = clamp(market.hype + 0.008);
  }
}

function maybeCommunityMessage(state: GameState): void {
  if (random(state) < 0.62) addFeed(state, "holder", "HOLDER", pick(state, HOLDER_LINES));

  if (state.market.price > state.priceHistory[0]!.price * 1.7 && random(state) < 0.42) {
    addFeed(state, "market", "MARKET", "Momentum scanners are picking up the token. Volume is accelerating.");
    state.market.hype = clamp(state.market.hype + 0.04);
  }

  if (state.market.suspicion > 0.72 && random(state) < 0.5) {
    addFeed(state, "holder", "HOLDER", "I am not saying rug. I am saying I want the wallet explanation before the next green candle.");
  }
}
...
function advanceBeat(state: GameState): void {
  state.beat += 1;
  if (state.beat % 4 === 0) {
    state.player.attention = Math.min(state.player.maxAttention, state.player.attention + 1);
  }
  ...
  if (state.market.price < 0.11 && state.market.confidence < 0.14) {
    ...
    addFeed(state, "market", "MARKET", "The token is functionally dead. No single rug event — just no bid left.");
    note(state, "Project collapsed without a single catastrophic liquidity-removal event.");
    return;
  }

  if (state.beat >= 32) {
    state.status = "survived";
    addFeed(state, "system", "SESSION", "The token survived the high-risk launch window. Episode complete.");
    note(state, "Reached the end of the launch window without a terminal rug event.");
  }
}
...
export function createGame(seedInput: string | number = Date.now()): GameState {
  ...
  const archetypes: Archetype[] = ["sociopath", "true_believer", "desperate_dev", "accidental_success"];
  ...
  const state: GameState = {
    ...
    market: {
      ...
      contractRisk: archetype === "sociopath" ? 0.72 : 0.28 + random(bootstrap) * 0.34,
    },
    player: {
      tokens: 1_000,
      cash: 250,
  ...
  addFeed(state, "system", "POSITION", `You bought $1,000 of ${state.token.symbol}. You are already down 7%.`);
  addFeed(state, "dev", "DEV", "Welcome. Ignore the noise, read what we ship, and do not confuse volatility with failure.");
  addFeed(state, "holder", "HOLDER", "early crew. either we are geniuses or this becomes a screenshot.");
  note(state, `Creator archetype initialized as ${archetype}; player cannot see this during play.`);
...
export function getPublicState(state: GameState): PublicGameState {
  ...
  return {
    ...
    pnlPercent: ((value - state.player.costBasis) / state.player.costBasis) * 100,
...
export function summarizeGame(state: GameState): GameSummary {
  if (state.status === "playing") throw new Error("Cannot summarize an active game.");

  const finalValue = portfolioValue(state);
  const pnlPercent = ((finalValue - state.player.costBasis) / state.player.costBasis) * 100;
  const capturePercent = state.peakPortfolio <= 0 ? 0 : (finalValue / state.peakPortfolio) * 100;

  let verdict = "You survived the launch window.";
  if (state.status === "exited") verdict = pnlPercent >= 0 ? "You got out with the bag intact." : "You escaped, but paid tuition.";
  if (state.status === "rugged") verdict = "The creator got to the exit first.";
  if (state.status === "collapsed") verdict = "No cinematic rug. The market simply died.";
  if (state.status === "survived" && state.creator.rugAbandoned) verdict = "The project survived — and the creator's plan changed along the way.";
