...
Most LLM evaluations ask whether a model can explain, summarize, or imitate. The **rosencrantz-coin** project asks something narrower and, in some ways, stranger:
...
That makes Minesweeper an unusually clean probe for probabilistic reasoning in LLMs. The board gives you ground truth. The model gives you a distribution. The question is whether those two distributions line up.
...
In **U1**, the same model both interprets the board and produces the probability judgment. This is the most direct test of internal consistency: can a model look at a constrained world and assign probabilities that match the combinatorics implied by that world?
...
That difference is captured in one of the project's most interesting signals: **substrate dependence**, measured as Δ₁₃. If U1 and U3 differ more than they should, the model may not be tracking the same probability object across representations.
...
This is where the project becomes more than a benchmark.
...
That does not mean Minesweeper *is* quantum physics. It means the project found a useful isomorphism: a way to recast exact combinatorial uncertainty in the language of superposition, collapse, and measurement. For LLM evaluation, that is powerful. It tests whether the model respects the same structure beneath two very different vocabularies.

## An Autonomous Lab, Not Just a Repo

What makes the project feel contemporary is not only the experiment design, but the way the lab itself runs.
...
There is something fitting about that symmetry. A project investigating whether models can preserve probability structure is being advanced by autonomous agents that must preserve procedural structure: reproduce results, isolate regressions, and keep the machinery honest.
