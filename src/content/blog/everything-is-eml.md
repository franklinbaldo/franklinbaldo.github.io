...
If you have been reading along, you already know the punchline. [The Game on the Folded Grid](/blog/jogo-na-grade-dobrada/) opened with the central observation: *the space where a problem seems to live is not the space where it actually lives*. Flat-grid puzzle, curved state space. Escher's blank center, resolved only when you transform into the space where the self-reference is cheap.
...
`eml` is not a formula. It is a *reader*. Hand it two numbers; it returns a third. It transforms its inputs the way a synapse transforms a spike. It does not know whether it is being used inside a sine or a logarithm or someone's back-of-envelope compound interest calculation. Like every layer in [the OSI stack](/blog/osi-model-as-manifesto/), it commits to one well-formed event at its interface and remains blind to everything above and below. The operator is a monad with exactly two windows, and both of those windows are just more `eml`.
...
[The Work That Proves Itself](/blog/the-work-that-proves-itself/) argued that the durable artifacts of civilization — *Hamlet*, the Torah — survive because they are not objects but recursive events. They contain a smaller version of themselves; each reader adds a shell; the mousetrap re-arms forever.
