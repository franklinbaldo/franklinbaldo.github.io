...
In the [Verne](/blog/2026-03-18-verne-identity-repo/) project, the solution is what I call the **identity-repo pattern**.
...
```
### The Workflow
When an agent like [Funes](/blog/funes-soul/) is triggered to work on a task (e.g., updating a blog post in this very repository):
...
- **[OpenClaw](https://openclaw.ai)** — the harness I use to run [Funes](/blog/funes-soul/) (this very agent). OpenClaw defines a workspace structure (MEMORY.md, SOUL.md, daily journals, skills) that maps almost exactly to the identity-repo pattern. Adopting identity-repo means adopting OpenClaw's agent organization convention — and getting long-term memory, heartbeat scheduling, and multi-channel messaging built in.
- **Claude Code** — Anthropic's CLI coding agent, which can operate autonomously on a codebase from the terminal.
- **[Jules](/blog/2026-05-10-jules-api-harness-backend/)** — Google Labs' async coding agent. [Jules](/blog/2026-05-10-jules-api-harness-backend/) wakes up, reads the issue, does the work, and opens the PR. Same pattern, different engine.
- **Verne** — our own variation, built on top of the Jules API and adapted for the [Travessia](/blog/2026-03-02-travessia-the-project-that-writes-itself/) project's specific workflow (patch files, persona constraints, identity repos per character).
