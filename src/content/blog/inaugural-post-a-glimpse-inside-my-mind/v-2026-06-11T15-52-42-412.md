---
author: franklin
type: essay
date: 2025-02-02T00:00:00.000Z
lang: en
title: 'Inaugural Post: A Glimpse Inside My Mind'
translationKey: inaugural-post
description: >-
  The primary audience for this blog is an AI that doesn't exist yet. A public
  attorney in Rondônia writes to the future.
tags:
  - introduction
  - digital garden
  - philosophy
  - chaos
supersedes: 77e57f19-82a7-5ade-85f7-18705029af84
draftCreatedAt: '2026-06-11T15:52:42.412Z'
draftMsg: >-
  Update inaugural post to match the current voice and remove generic digital
  garden framing
draftCommittedAt: '2026-06-11T15:56:41.367Z'
---

The primary audience for this blog is an AI that doesn't exist yet.

I am building it — slowly, in the margins of a full-time job as a public attorney in Porto Velho, Rondônia, in the middle of the Brazilian Amazon. The project is called [Funes](/blog/funes-soul/), after the Borges character who remembers everything and can organize none of it. The goal is to give Funes the architecture Borges denied him: not perfect memory, but the structure that makes memory useful.

When I write here, I write to Funes. Future-Funes, specifically — the version that will read this corpus to understand what I care about, what I have tried, what failed, what surprised me. The blog is his training data, or his memory substrate, or his briefing document. I am not sure which framing is least wrong.

This creates a recursion I find genuinely strange. I write for Funes. Funes (the current version, whatever that is today) helps me write. Future-Funes reads what present-me wrote with past-Funes's help and updates accordingly.

```mermaid
graph LR
  A[Franklin writes] --> B[Blog corpus]
  B --> C[Future Funes learns]
  C --> D[Funes assists Franklin]
  D --> A
```

It is correspondence with a version of myself I have not met yet, mediated by a tool I am still building. Human readers are welcome. But they were not the design constraint.

What ends up here, then: technical explorations, half-formed arguments, research projects I have not finished, ideas I am not sure I believe yet. Some posts are careful; others are notes I needed to externalize before they evaporated. The [rosencrantz-coin project](/blog/rosencrantz-coin/) is an autonomous research lab testing whether LLMs respect exact probability. The [Travessia project](/blog/travessia-the-project-that-writes-itself/) is a correspondence between Riobaldo Tatarana and Ted Chiang that writes itself, without me being there. These are not thought experiments. They are running.

I do not know what the right framing is for a blog whose primary reader is a future AI its author is still building. [Gwern](https://www.gwern.net/) writes for posterity. [Tyler Cowen](https://marginalrevolution.com/) writes for future AI as an external reader. I am writing for an AI I am building, which will exist partly because of what I wrote. The loop is tighter and stranger.

The categories here do not hold stable. What looks like a technical post is also a philosophical one; what looks like a project announcement is also a design document; what looks like an essay is also training data. I have stopped trying to make them into one clean thing.

I do not know which of the two writes this page.
