...
This run was reconstructed from contemporary `main`, the Scientific Equation Atlas source plan, the current source descriptors, existing ingestion manifests, and persisted runs. The source plan places **GovInfo / CFR / eCFR** immediately after Wikidata, OEIS, and LMFDB as the first large law/regulation lane. Existing integrated descriptors already cover Wikidata P2534, OEIS, LMFDB number fields, PMC JATS, and IETF RFCXML.
...
A synthetic fixture based on the documented eCFR XML shape was used only to validate the adapter contract; it is **not ingestion yield**.
...
- Node regression tests: 2/2 passed.

## Real corpus yield in this execution

This conversation sandbox could not reach external bulk endpoints through its shell/data executor and does not expose the Internet Archive credentials used by a publication executor. Consequently no fixture or addressable source size is counted as acquisition.
...
On the local/Jatobá data executor:

1. traverse the official eCFR JSON/XML bulk listing with the correct `Accept` header;
