...
blocking_constraint: "The immediate release boundary remains internal implementation, CI, security and deployment debt, but it narrowed materially again on 2026-09-23. PRs #1746 and #1747 merged the bounded linear email validator and fail-closed npm-audit gate with the critical tar fix; PR #1748 then removed all 22 high-severity production dependency findings, leaving 10 high findings documented as DEV-only plus 15 moderate and 2 low findings. The README still explicitly blocks Android 1.0 on real TLS pinning, Android 14 critical-push wakeup behavior, a broad cross-tenant route audit, branch-protection hardening, missing mobile-release secrets and a failing deploy configuration. Those are known internal gates before independent physical-device field review becomes the immediate missing resource."
...
trajectory:
  ...
  - "2026-09-23: #1746 and #1747 merge, then #1748 reduces npm-audit high findings from 22 to 10 DEV-only and reports production high vulnerabilities at 0, while the README's mobile, tenant-isolation, branch and deploy gates remain explicit."
